...
{
	std::cout << "parseArgument" << std::endl;
	int option;
	float foption;
	char buf[1000];
...
void imuCallback(const sensor_msgs::Imu::ConstPtr &msg)
{
	...
	m_acc.push_back(acc);
	imu_time_stamp.push_back(msg->header.stamp.toSec());

	if (m_gry.size() > 1000)
	{
		size_t remove_imus = m_gry.size() - 1000;
		m_gry.erase(m_gry.begin(), m_gry.begin() + remove_imus);
		m_acc.erase(m_acc.begin(), m_acc.begin() + remove_imus);
		imu_time_stamp.erase(imu_time_stamp.begin(), imu_time_stamp.begin() + remove_imus);
	}
}
...
std::cout << "4" << std::endl;
    ...
    if(undistorter->photometricUndist != 0)
    	fullSystem->setGammaFunction(undistorter->photometricUndist->getG()); // TODO what does this mean
...
//     runthread.join();
	...
	ros::NodeHandle nh;
    ros::Subscriber imgSub = nh.subscribe("/cam0/image_raw", 1, &vidCb);
  	ros::Subscriber imuSub = nh.subscribe("/imu0", 1, &imuCallback);
